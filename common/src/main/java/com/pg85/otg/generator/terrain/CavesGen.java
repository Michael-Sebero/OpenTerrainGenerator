...
{
    private final WorldConfig worldSettings; // Made final
    ...
    private void generateCaveNode(long seed, ChunkBuffer generatingChunkBuffer, double x, double y, double z, float paramFloat1, float paramFloat2, float paramFloat3, int angle, int maxAngle, double paramDouble4)
    {
        ...
        for (; angle < maxAngle; angle++)
        {
            ...
            // Check if distance to working point (x and z) too larger than working radius
            double d5 = x - real_x;
            double d6 = z - real_z;
            ...
            // OPTIMIZATION: Use Math.max/min instead of if statements
            m = Math.max(0, m);
            n = Math.min(16, n);
            maxDepth = Math.max(1, maxDepth);
            minDepth = Math.min(this.worldSettings.worldHeightCap - 8, minDepth);
            i3 = Math.max(0, i3);
            i4 = Math.min(16, i4);
            ...
            for (int local_x = m; local_x < n; local_x++)
            {
                ...
                {
                    for (int local_y = minDepth + 1; local_y >= maxDepth - 1; local_y--)
                    {
                        if (local_y >= 0 && local_y < this.worldSettings.worldHeightCap)
                        {
                            ...
                            {
                                waterFound = true;
                                break searchLoop; // Use labeled break
                            }
                ...
                for (int local_z = i3; local_z < i4; local_z++)
                {
                    ...
                    if (d9 * d9 + d10 * d10 < 1.0D)
                    {
                        ...
                        for (int currentDepth = minDepth; currentDepth > maxDepth; currentDepth--)
                        {
                            ...
                            if ((materialAbove.isAir() || materialAbove.isLiquid()) && 
                                !(material.isAir() || material.isLiquid()))
                            {
    ...
    private boolean isSuitableBlock(LocalMaterialData material, LocalMaterialData materialAbove)
    {
        if (material.isSolid())
        {
            return true;
    ...
    protected void generateChunk(ChunkCoordinate chunkCoord, ChunkBuffer generatingChunkBuffer)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            double x = chunkCoord.getBlockX() + this.random.nextInt(ChunkCoordinate.CHUNK_SIZE);
            ...
            double z = chunkCoord.getBlockZ() + this.random.nextInt(ChunkCoordinate.CHUNK_SIZE);
